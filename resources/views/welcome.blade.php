                    ...
                    <div id="text" class="col-sm-11">
                        @foreach($article as $item)
                            <div class="col-sm-12">
                                ...
                                <div class="col-sm-2"><img src="{{asset('img/'.$item->thumbnail)}}" width="100px" height="100px" alt=""></div>
                                <div class="col-sm-10" {{--style="overflow: hidden;text-overflow: ellipsis;white-space: nowrap;"--}}>
                                    <textarea name="" id="" cols="140" rows="5">{!! strip_tags($item->content) !!}</textarea>
                                </div>
                            </div>
                        @endforeach
                    </div>
        ...
        function ChangeCate(cate_name) {
            $.get("{{url('chcate')}}"+"/"+cate_name,function (data) {
                ...
                for(item in data)
                {
                    ...
                    text += '<div class="col-sm-2"><img src="{{asset('img/')}}'+"/"+data[item]['thumbnail']+'" width="100px" height="100px" alt=""></div>';
                    text += '<div class="col-sm-10"><textarea name="" id="" cols="140" rows="5">'+data[item]['content'].replace(/<[^>]+>/g,"")+'</textarea></div>';
                    text += '</div>';
                }
                $('#text').append(text);
            })
